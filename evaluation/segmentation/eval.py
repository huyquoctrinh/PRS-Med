...
def evaluate(opt, args):
    ...
    for dataset in datasets:
        ...
        gt_root = os.path.join(opt.Eval.gt_root, dataset, "test_masks")
        ...
        for i, sample in samples:
            ...
            pred_mask = cv2.imread(os.path.join(pred_root, pred), 0)
            gt_mask = cv2.imread(os.path.join(gt_root, gt), 0)
