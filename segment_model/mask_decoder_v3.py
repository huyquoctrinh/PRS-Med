...
class PromptedMaskDecoder(nn.Module):
    def __init__(self, prompt_dim=4096, image_dim=256, hidden_dim=512):
        ...
        for i in range(3):
            nn.init.xavier_uniform_(self.transformer_decoder.layers[i].self_attn.in_proj_weight)
            nn.init.xavier_uniform_(self.transformer_decoder.layers[i].self_attn.out_proj.weight)
            nn.init.zeros_(self.transformer_decoder.layers[i].self_attn.in_proj_bias)
            nn.init.zeros_(self.transformer_decoder.layers[i].self_attn.out_proj.bias)
            nn.init.xavier_uniform_(self.transformer_decoder.layers[i].linear1.weight)
            nn.init.zeros_(self.transformer_decoder.layers[i].linear1.bias)
            nn.init.xavier_uniform_(self.transformer_decoder.layers[i].linear2.weight)
            nn.init.zeros_(self.transformer_decoder.layers[i].linear2.bias)
            nn.init.ones_(self.transformer_decoder.layers[i].norm1.weight)
            nn.init.zeros_(self.transformer_decoder.layers[i].norm1.bias)
            nn.init.ones_(self.transformer_decoder.layers[i].norm2.weight)
            nn.init.zeros_(self.transformer_decoder.layers[i].norm2.bias)
        ...
        # self.up1 = 
        self.out_dec = nn.Conv2d(image_dim // 16, 1, 1)
    ...
    def forward(self, image_feat, prompt_feat):
        ...
        fused, _ = self.attn(image_flat, prompt_proj, prompt_proj)  # (B, H*W, hidden_dim)
        fused = self.norm1(fused)  # (B, H*W, hidden_dim)
        fused = self.ffn(fused)  # (B, H*W, hidden_dim)

        attn_out = self.transformer_decoder(image_flat, fused) # (B, H*W, hidden_dim)
        ...
        mask = self.out_dec(mask)  # (B, 1, 128, 128)
        # mask = F.interpolate(mask, scale_factor=8, mode='bilinear', align_corners=True)  # (B, 1, 64, 64)
        # print(mask.shape)
        return mask
