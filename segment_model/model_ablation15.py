...
from llava_ablation.utils import disable_torch_init
from llava_ablation.mm_utils import process_images, tokenizer_image_token, get_model_name_from_path, KeywordsStoppingCriteria
...
class LLMSeg(nn.Module):
    def __init__(
        ...
        self.tokenizer, self.base_model, self.image_processor, self.context_len = load_pretrained_model(
            "/home/mamba/ML_project/Testing/Huy/llm_seg/weight/model_16",
            model_base,
            model_name,
            load_8bit,
            load_4bit,
    ...
    def generate(
        ...
        with torch.no_grad():
            ...
            # image_size = image_tensor_for_image_enc.shape[-2:]
            # print("Image size:", image_tensor_for_image_enc.shape[-2:])
            prompt_embedding = self.base_model.extract_last_hidden_state(
                ...
                images = image_tensor_for_vlm,
                image_sizes = image_tensor_for_image_enc.shape[-2:],
            )["hidden_states"][-1]
